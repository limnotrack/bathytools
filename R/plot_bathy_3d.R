...
plot_bathy_3d <- function(x, fact = 0, cols = "topo", split_lake = FALSE,
                           mask_to_lake = FALSE,
  ...
  scene <- list(camera = list(eye = list(x = -0.5, y = -0.75, z = 1.25)),
               aspectmode = "manual", aspectratio = list(x = 1, y = 1,
                                                         z = aspectratio_z),
               xaxis = list(title = "Easting", tickformat = ".7r", nticks = 5),
               yaxis = list(title = "Northing", tickformat = ".7r", nticks = 5),
               zaxis = list(title = "Depth (m)", nticks = 5))
  ...
  if (!split_lake) {
    ...
    ras_agg <- terra::extend(ras_agg, 10)

    ras_agg[is.na(ras_agg[])] <- 0

    ras_agg <- terra::mask(ras_agg, sf::st_buffer(nz_shore, buff * 15),
                           touches = FALSE)
